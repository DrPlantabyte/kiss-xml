...
impl ParseTree {
	...
	pub fn push(&mut self, new_element: Element) {
		if self.pos.is_none() {
			self.data.insert(
				0,
				ParseTreeNode { id: 0, value: Box::new(new_element), parent_id: None, child_ids: Vec::new() },
			);
			self.pos = Some(0);
		} else {
			let old_pos = self.pos.unwrap();
			let new_pos = self.data.len();
			self.data.get_mut(&old_pos).expect("logic error").child_ids.push(new_pos);
			self.data.insert(
				new_pos,
				ParseTreeNode {
					id: new_pos,
	...
	pub fn pop(&mut self) -> Result<(), KissXmlError> {
		...
		let new_pos = self.data.get(&pos).expect("logic error").parent_id;
		self.pos = new_pos;
		Ok(())
	}
	...
	pub fn append(&mut self, n: impl Node + 'static) -> Result<(), KissXmlError> {
		...
		self.data.get_mut(&pos).expect("logic error").child_ids.push(new_id);
		self.data.insert(
			new_id,
			ParseTreeNode { id: new_id, value: Box::new(n), parent_id: Some(pos), child_ids: Vec::new() },
		);
		Ok(())
	}
	...
	pub fn top_element(&self) -> Option<&Element> {
		match self.pos {
			None => None,
			Some(pos) => Some(self.data.get(&pos).expect("logic error").value.as_element().expect("logic error")),
		}
	}
	...
	pub fn to_dom(mut self) -> Result<Element, KissXmlError> {
		...
		for i in (1..self.data.len()).rev() {
			...
			self.data
				...
				.expect("logic error: parent is missing")
				.value
				.as_element_mut()
				.expect("logic error: parent is not an Element")
...
#[cfg(test)]
mod tests {
	use crate::parsing::ElasticReader;
	use std::io;
